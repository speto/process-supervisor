...
class FakePlatform implements ProcessPlatform {
  ...
  async probeMany(pids: readonly number[]): Promise<ReadonlyMap<number, ProcessProbe>> {
    this.probeStarted?.();
    if (this.probeGate) await this.probeGate;
    if (this.inspectionError) throw this.inspectionError;
...
async function createLogs(root: string, processId = 'owned'): Promise<{stdoutPath: string; stderrPath: string}> {
  const directory = join(root, 'logs', Buffer.from(processId, 'utf8').toString('base64url'));
  await mkdir(directory, {recursive: true, mode: 0o700});
  const stdoutPath = join(directory, `${LAUNCH_ID}.stdout.log`);
  const stderrPath = join(directory, `${LAUNCH_ID}.stderr.log`);
  await Promise.all([
...
test('keeps ownership unresolved when batched process probing fails', async () => {
  const store = new MemoryRecordStore([record()]);
  const platform = new FakePlatform();
...
test('serializes concurrent reconciliation and creates one durable-log follower set', async () => {
  ...
  const {stdoutPath} = await createLogs(root);
  const store = new MemoryRecordStore([record()]);
  const platform = new FakePlatform();
...
test('close waits for in-flight reconciliation and cannot finish before ownership is resolved', async () => {
  ...
  await createLogs(root);
  const store = new MemoryRecordStore([record({shutdownPolicy: 'terminate'})]);
  const platform = new FakePlatform();
  platform.inspection = inspection();
  let releaseProbe!: () => void;
  let probeStarted!: () => void;
  const started = new Promise<void>((resolve) => { probeStarted = resolve; });
  platform.probeStarted = probeStarted;
  platform.probeGate = new Promise<void>((resolve) => { releaseProbe = resolve; });
  const supervisor = new ProcessSupervisor({
    stateDirectory: root,
    recordStore: store,
    ownershipLease: null,
