...
async function waitForAsync<T>(predicate: () => Promise<T | undefined>, timeoutMs = 2_000): Promise<T> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
...
test('rejects malformed JavaScript-facing process specifications with INVALID_SPEC', async () => {
  const root = await mkdtemp(join(tmpdir(), 'process-supervisor-invalid-js-'));
  const supervisor = new ProcessSupervisor({stateDirectory: join(root, 'state')});
  try {
    const malformed = {
      id: 'invalid-js',
      ...
      args: [],
      cwd: root,
      environment: {values: {TOKEN: 123}},
    } as unknown as ManagedProcessSpec;
    await assert.rejects(
      supervisor.start(malformed),
      (error: unknown) => error instanceof ProcessSupervisorError && error.code === 'INVALID_SPEC',
    );
  } finally {
    await supervisor.close();
...
test('requires an explicit ownership lease decision for custom record stores', async () => {
  const root = await mkdtemp(join(tmpdir(), 'process-supervisor-custom-store-'));
  try {
    const store = new FileProcessRecordStore(join(root, 'state'));
    assert.throws(
      () => new ProcessSupervisor({stateDirectory: join(root, 'state'), recordStore: store}),
      (error: unknown) => error instanceof ProcessSupervisorError && error.code === 'INVALID_SPEC',
    );
  } finally {
    await rm(root, {recursive: true, force: true});
...
test('retains ownership and refuses unsafe residual-scope retry after leader exit', async () => {
  ...
  const supervisor = new ProcessSupervisor({
    stateDirectory,
    recordStore: store,
    ownershipLease: new FileProcessOwnershipLease(stateDirectory),
    platform,
  ...
  try {
    ...
    await assert.rejects(
      supervisor.stop('residual'),
      (error: unknown) => error instanceof ProcessSupervisorError && error.code === 'PROCESS_CONTROL_UNCERTAIN',
    );
    assert.equal((await store.get('residual'))?.kind, 'valid');

    const groupId = Number(running.scope.id);
    try {
      process.kill(-groupId, 'SIGKILL');
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ESRCH') throw error;
    }
    await waitForAsync(async () => !await platform.isScopeAlive(running.scope!) ? true : undefined, 3_000);
...
test('failed close keeps preserve runtimes controllable for retry', async () => {
  const root = await mkdtemp(join(tmpdir(), 'process-supervisor-close-transaction-'));
  const platform = new FailableTerminationPlatform();
  const supervisor = new ProcessSupervisor({
    stateDirectory: join(root, 'state'),
    platform,
  ...
  try {
    await supervisor.start({
      id: 'preserved',
      executable: process.execPath,
      args: ['-e', 'console.log("preserved"); setInterval(()=>{},1000)'],
      cwd: root,
      ioMode: 'durable-log',
    ...
    await assert.rejects(
      supervisor.close(),
      (error: unknown) => error instanceof ProcessSupervisorError && error.code === 'PROCESS_STOP_FAILED',
    );

    assert.equal(supervisor.getSnapshot('preserved')?.state, 'running');
    await supervisor.readOutputTail('preserved', 'stdout');

    platform.failTermination = false;
    await supervisor.stop('preserved');
    await supervisor.stop('terminate-fails');
    await supervisor.close();
  } finally {
    platform.failTermination = false;
    await supervisor.close().catch(() => undefined);
...
test('closed supervisors reject lifecycle operations with SUPERVISOR_CLOSED', async () => {
  const root = await mkdtemp(join(tmpdir(), 'process-supervisor-closed-'));
  const supervisor = new ProcessSupervisor({stateDirectory: join(root, 'state')});
  try {
    await supervisor.close();
    assert.throws(
      ...
      (error: unknown) => error instanceof ProcessSupervisorError && error.code === 'SUPERVISOR_CLOSED',
    );
  } finally {
    await rm(root, {recursive: true, force: true});
  }
...
test('bounds each durable log stream while supervised', async () => {
  const root = await mkdtemp(join(tmpdir(), 'process-supervisor-log-bytes-'));
  const stateDirectory = join(root, 'state');
  const supervisor = new ProcessSupervisor({
    stateDirectory,
    maxDurableLogBytes: 1024,
    logPollMs: 10,
  });
  ...
  try {
    ...
    const encoded = Buffer.from('bounded-log', 'utf8').toString('base64url');
    const directory = join(stateDirectory, 'logs', encoded);
    const stdoutName = (await readdir(directory)).find((name) => name.endsWith('.stdout.log'));
    assert.ok(stdoutName);
    const stdoutPath = join(directory, stdoutName);
    await waitForAsync(async () => (await stat(stdoutPath)).size <= 1024 ? true : undefined, 3_000);

    await supervisor.stop('bounded-log');
  } finally {
    await supervisor.close();
    await rm(root, {recursive: true, force: true});
  }
